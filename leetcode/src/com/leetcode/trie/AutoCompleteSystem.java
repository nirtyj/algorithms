...
public class AutoCompleteSystem {
	...
	private void add(String word, int count) {
		...
		for (char c : word.toCharArray()) {
			TrieNode n = curr.children.getOrDefault(c, new TrieNode());
			int val = n.counts.getOrDefault(word, 0);
			val = val + count;
			n.counts.put(word, val);
	...
	public List<String> input(char c) {
		if (c == '#') {
			add(previousSearchString, 1);
			previousSearchString = "";
			return Collections.emptyList();
		}

		previousSearchString = previousSearchString + c;
		TrieNode curr = root;
		for (char cc : previousSearchString.toCharArray()) {
