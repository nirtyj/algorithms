...
public class ExcelSum {
	...
	private class Cell {
		int val = 0;
		HashMap<Cell, Integer> formula = new HashMap<>();// one cell might appear multiple times. while summing, multiply the val * times.
		...
		private void addFormulaCell(int r, int c) {
			...
			int val = formula.getOrDefault(rangeCell, 0);
			val++;
			formula.put(rangeCell, val);
		}
