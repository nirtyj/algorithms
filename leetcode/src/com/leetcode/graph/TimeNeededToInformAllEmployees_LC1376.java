 ...
 */
public class TimeNeededToInformAllEmployees_LC1376 {
    ...
    public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
        ...
        for(int i=0;i<manager.length;i++){
            ...
            List<Integer> list = subs.getOrDefault(manager[i], new ArrayList<>());
            list.add(i);
            subs.put(manager[i], list);
        }
        return dfs(subs, informTime, headID);
    }

    private int dfs(HashMap<Integer, List<Integer>> subs, int[] informTime, int manager){
        int time = informTime[manager];
        int subTime = 0;
        if(subs.containsKey(manager) && subs.get(manager).size()>0){
            for(Integer sub: subs.get(manager)){
                subTime = Math.max(subTime, dfs(subs, informTime, sub));
            }
        }
        return time + subTime;
    }
